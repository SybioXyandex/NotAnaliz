...
const subjectWeights: { [key: string]: number } = {
...
// --- STATE MANAGEMENT ---
const appState = {
  currentView: 'login' as 'login' | 'dashboard' | 'studentDashboard',
  user: null as User | null,
  authView: 'login' as 'login' | 'register',
  authError: null as string | null,
  userRole: 'none' as 'teacher' | 'student' | 'none',
  dashboardView: 'analysis' as 'analysis' | 'upload' | 'history' | 'students',
  studentDashboardView: 'results' as 'results' | 'analysis' | 'history',
  isLoading: false,
  analysisResult: '',
  studentAnalysisResult: '',
  uploadedData: null as string[][] | null,
  uploadedFileName: '',
  dataForAnalysis: '',
  analysisHistory: [] as AnalysisHistoryItem[],
  viewingHistoryItemId: null as number | null,
  selectedStudentName: null as string | null,
  selectedSubjectName: null as string | null,
  teacherNote: '',
  isNoteLoading: false,
  dbError: null as { context: string; html: string } | null,
};
...
async function getStudentSubjectHistory(studentNo: string, subject: string) {
...
function handleDownloadWord(elementId: string, fileName: string, buttonId: string) {
    const downloadButton = document.getElementById(buttonId) as HTMLButtonElement;
...
// --- RENDER FUNCTIONS ---
function renderScoreChart(student: { [key: string]: any }, scoreHeaders: string[]) {
    if (chartInstance) {
...
function renderSubjectHistoryChart(historyData: { score: number, created_at: string }[], subjectName: string) {
    if (chartInstance) {
        chartInstance.destroy();
        chartInstance = null;
    }

    const canvas = document.getElementById('subjectHistoryChart') as HTMLCanvasElement;
    if (!canvas) return;
    const ctx = canvas.getContext('d');
    if (!ctx) return;
...
function renderLogin() {
    ...
    document.getElementById('register-role')?.addEventListener('change', (e) => {
        ...
        const studentNumberInput = document.getElementById('register-student-number') as HTMLInputElement;
...
function renderStudentListPage(container: HTMLElement, studentList: any[]) {
    ...
    document.querySelectorAll('.student-list-item button').forEach(button => {
        button.addEventListener('click', (e) => {
            ...
            if (targetButton) {
                ...
                const studentName = (targetButton as HTMLButtonElement).dataset.studentName;
                if (studentName) {
...
async function renderStudentProfilePage(container: HTMLElement, studentName: string, studentData: { scoreHeaders: string[], studentList: any[], subjectAverages: { [key: string]: number } }) {
    ...
    document.querySelectorAll('.view-history-btn').forEach(btn => btn.addEventListener('click', (e) => {
        const button = (e.target as HTMLElement).closest('.view-history-btn');
        if (button) {
    ...
    document.querySelectorAll('.subject-name-btn').forEach(button => {
        button.addEventListener('click', (e) => {
            const targetButton = (e.target as HTMLElement).closest('.subject-name-btn');
            if (targetButton) {
                const subjectName = (targetButton as HTMLButtonElement).dataset.subjectName;
                if (subjectName) {
                    if (chartInstance) {
                        chartInstance.destroy();
                        chartInstance = null;
...
async function renderStudentsView(container: HTMLElement) {
    ...
    if (appState.dashboardView === 'analysis') {
        ...
        if (listContainer && detailContainer && listCard) {
            if (appState.viewingHistoryItemId !== null) {
                ...
                listContainer.querySelectorAll<HTMLButtonElement>('.view-history-btn').forEach(btn => {
    ...
    if (appState.studentDashboardView === 'results') {
        if (appState.selectedSubjectName) {
            ...
            document.querySelectorAll('.subject-name-btn').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    const button = (e.target as HTMLElement).closest('.subject-name-btn');
                    if (button) {
                        const subjectName = (button as HTMLButtonElement).dataset.subjectName;
                        if (subjectName) {
                            appState.selectedSubjectName = subjectName;
                            render();
                        }
...
async function handleRegisterSubmit(e: Event) {
    ...
    const studentNumber = (document.getElementById('register-student-number') as HTMLInputElement).value;
...
function handleMenuViewChange(view: 'analysis' | 'upload' | 'history' | 'students') {
...
function handleStudentMenuViewChange(view: 'results' | 'analysis' | 'history') {
...
async function saveAnalysisToHistory(summary: string, result: string) {
...
async function fetchTeacherNote(studentName: string) {
    if (!supabase || !appState.user) return;
...
async function handleSaveTeacherNote(studentName: string) {
    if (!supabase || !appState.user) return;
    const noteInput = document.getElementById('teacher-note-input') as HTMLTextAreaElement;
    if (!noteInput) return;
...
function getDbErrorInstructions(error: any): { context: string; html: string } {
    const message = (error?.message || '').toLowerCase();
    
    const createTableHTML = (tableName: string, sql: string) => `
...
USING (
  (
    ...
    const rlsPolicyHTML = (tableName: string, operation: 'insert' | 'select' | 'update' | 'delete') => {
